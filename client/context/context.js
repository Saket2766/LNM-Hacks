...
export const DiscordProvider = ({ children }) => {
    ...
    const checkIfWalletIsConnected = async () => {
        ...
        try {
            const addrerssArray = await window.ethereum.request({ method: 'eth_accounts' })
            if (addrerssArray.lenght > 0) {
                setCurrentAccount(addrerssArray[0]);
                createUserAccount(addrerssArray[0]);
            } else {
